...
impl MemoryDataAccess {
    ...
    pub async fn get_memory_corpus_by_user_id(
        user_id: &str,
    ) -> Result<Option<MemoryCorpus>> {
        let db = get_db();
        let mut db = db.write().await;
        
        db.metrics.reads += 1;
        
        let result = db.memory_corpus
            .iter()
    ...
    pub async fn add_memory_fragment(fragment: &MemoryFragment) -> Result<Uuid> {
        ...
        if let Some(existing) = db.memory_fragments.iter_mut().find(|f| f.id == new_fragment.id) {
    ...
    pub async fn search_memory_fragments(
        ...
        let results: Vec<_> = db.memory_fragments
            .iter()
            .filter(|f| f.user_id == query.user_id)
            .filter(|f| query.memory_types.as_ref().map_or(true, |types| types.contains(&f.memory_type)))
            .filter(|f| query.keywords.as_ref().map_or(true, |kws| 
                kws.iter().any(|kw| f.keywords.contains(kw) || f.content.contains(kw))
            ))
            .filter(|f| query.min_importance.map_or(true, |min| f.importance_score >= min))
            .filter(|f| query.time_range.as_ref().map_or(true, |tr| 
    ...
    pub async fn get_user_interactions(
        ...
        let mut logs: Vec<_> = db.interaction_logs
            .iter()
    ...
    pub async fn get_user_statistics(user_id: &str) -> Result<UserStatistics> {
        ...
        let memory_type_counts = db.memory_fragments
            .iter()
        ...
        let last_interaction = db.interaction_logs
            .iter()
            .filter(|log| log.user_id == user_id)
        ...
        Ok(UserStatistics {
            ...
            total_memories: db.memory_fragments.iter().filter(|f| f.user_id == user_id).count() as u64,
            memory_type_counts,
            total_interactions: db.interaction_logs.iter().filter(|log| log.user_id == user_id).count() as u64,
            last_interaction,
    ...
    fn validate_memory_fragment(fragment: &MemoryFragment) -> Result<()> {
        ...
        if fragment.importance_score < 0.0 || fragment.importance_score > 1.0 {
            return Err(anyhow::anyhow!("importance_score must be between 0.0 and 1.0"));
        }
