...
fn test_message_direction_serialization() {
    ...
    // 测试反序列化
    let deserialized: MessageDirection = serde_json::from_str("\"inbound\"").expect("反序列化失败");
    assert!(matches!(deserialized, MessageDirection::Inbound));

    let deserialized: MessageDirection = serde_json::from_str("\"outbound\"").expect("反序列化失败");
...
mod mock_tests {
    ...
    impl MemoryRepository for MockMemoryRepository {
        async fn save_memory_corpus(&self, _corpus: &sentio_memory::MemoryCorpus) -> MemoryResult<()> {
            Ok(())
        }

        async fn get_memory_corpus(&self, _user_id: &str) -> MemoryResult<Option<sentio_memory::MemoryCorpus>> {
            Ok(None)
        }
        ...
        async fn save_interaction(&self, _user_id: &str, interaction: &InteractionLog) -> MemoryResult<()> {
            let mut interactions = self.interactions.lock().unwrap();
            interactions.push(interaction.clone());
            Ok(())
        }

        async fn search_memories(&self, _query: &sentio_memory::MemoryQuery) -> MemoryResult<Vec<sentio_memory::MemoryFragment>> {
            Ok(Vec::new())
        }
        ...
        async fn get_user_statistics(&self, _user_id: &str) -> MemoryResult<sentio_memory::UserStatistics> {
